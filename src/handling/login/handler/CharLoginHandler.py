...
class CharLoginHandler:
    ...
    def login(self, slea: Any, c: Any) -> None:
        ...
        防万能 = False
        if not ip.lower() == "127.0.0.1".lower() and getS == s:
            防万能 = False
        ipBan = c.hasBannedIP()
        macBan = c.isBannedMac(macData)
        banned = ipBan or macBan or 防万能
        loginok = 0
        if not bool(ServerProperties.getProperty("RoyMS.AutoRegister")) or not AutoRegister.autoRegister or AutoRegister.getAccountExists(login) or banned:
            AutoRegister.success = True
            AutoRegister.mac = True
            loginok = c.login(login, pwd, ipBan or macBan or 防万能)
            tempbannedTill = c.getTempBanCalendar()
            if loginok == 0 and (ipBan or macBan or 防万能) and not c.isGm():
                loginok = 3
                if macBan or ipBan:
            ...
            return
        if pwd.lower() == "disconnect".lower() or pwd.lower() == "fixme".lower() or pwd.lower() == "admin".lower() or pwd.lower() == "000000".lower():
    ...
    def CheckCharName(self, name: str, c: Any) -> None:
        c.getSession().write(LoginPacket.charNameResponse(name, not MapleCharacterUtil.canCreateChar(name) or LoginInformationProvider.getInstance().isForbiddenName(name)))
    ...
    def CreateChar(self, slea: Any, c: Any) -> None:
        ...
        # switch (gender):
            # case 0:
                if face != 20100 and face != 20401 and face != 20402:
                    return
                if hair != 30030 and hair != 30027 and hair != 30000:
                    return
                if top != 1040002 and top != 1040006 and top != 1040010 and top != 1042167:
                    return
                if bottom != 1060002 and bottom != 1060006 and bottom != 1062115:
                    return
                break
            # case 1:
                if face != 21002 and face != 21700 and face != 21201:
                    return
                if hair != 31002 and hair != 31047 and hair != 31057:
                    return
                if top != 1041002 and top != 1041006 and top != 1041010 and top != 1041011 and top != 1042167:
                    return
                if bottom != 1061002 and bottom != 1061008 and bottom != 1062115:
                    return
                break
            # default:
                return
        if shoes != 1072001 and shoes != 1072005 and shoes != 1072037 and shoes != 1072038 and shoes != 1072383:
            return
        if weapon != 1302000 and weapon != 1322005 and weapon != 1312004 and weapon != 1442079:
            return
        ...
        if JobType == 0:
            newchar.setQuestAdd(MapleQuest.getInstance(20022), 1, "1")
            newchar.setQuestAdd(MapleQuest.getInstance(20010), 1, None)
        if MapleCharacterUtil.canCreateChar(name) and not LoginInformationProvider.getInstance().isForbiddenName(name):
            MapleCharacter.saveNewCharToDB(newchar, JobType, JobType == 1 and db == 0)
    ...
    def Character_WithoutSecondPassword(self, slea: Any, c: Any) -> None:
        charId = slea.readInt()
        if not c.isLoggedIn() or loginFailCount(c) or not c.login_Auth(charId):
            c.getSession().write(MaplePacketCreator.enableActions())
            return
        if ChannelServer.getInstance(c.getChannel()) is None or c.getWorld() != 0:
            c.getSession().close(True)
            return
